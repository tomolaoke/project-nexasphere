...
Backend: Ollama (free, local, open-source models) if reachable at
OLLAMA_HOST. If Ollama is not running -- e.g. a judge's machine without it
installed -- the system falls back automatically to the deterministic
template narrator. Nothing breaks and nothing is invented either way.
"""
...
def narrate_finding(finding) -> NarrationResult:
    ...
    generated = _call_ollama(_SYSTEM_PROMPT, user_prompt)
    if generated and _numbers_are_grounded(generated, finding.evidence) and _entities_are_grounded(generated, finding.evidence):
        return NarrationResult(text=generated, source="llm", verified=True)
    return NarrationResult(text=template_text, source="template", verified=True)
...
def narrate_answer(question: str, result: dict, template_answer: str) -> NarrationResult:
    ...
    generated = _call_ollama(_QA_SYSTEM_PROMPT, user_prompt)
    if generated and _numbers_are_grounded(generated, result) and _entities_are_grounded(generated, result):
        return NarrationResult(text=generated, source="llm", verified=True)
    return NarrationResult(text=template_answer, source="template", verified=True)
